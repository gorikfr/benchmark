...
# Explicitly group comparable runs (useful when combining speed and IQ results).
python3 llm_bench.py \
...
You normally do not need to provide an experiment ID. The script creates a per-model session ID automatically. If an invocation is interrupted, rerunning the same command resumes its unfinished session and, for lm-eval, its request cache. Once a session finishes, the next invocation gets a new ID. Use `--new-session` to force a fresh session, or `--experiment-id NAME` when you intentionally want to group comparable runs across commands or models.
